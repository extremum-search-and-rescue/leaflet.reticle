namespace L {
    export interface Map extends Evented {
        reticleControl: L.ReticleControl
    }
    ...
    export class ReticleControl extends L.Control {
        ...
        show(e: L.LayersControlEvent & { layer: L.Layer & { options: any } }) {
        ...
        hide(e: L.LayersControlEvent & { layer: L.Layer & { options: any } }) {
            if (e.layer && e.layer.options && e.layer.options.id === 'SC') {
        ...
        _updateReticle(context?: L.ReticleControl | {new:string, old: string}) {
            ...
            function _getScaleRatioLabel(maxDist: number): [number, string] {
            ...
            function getRect(xStart: number, yStart: number, width: number, height: number) {
                return `<rect class="gis-themeaware reticle ${theme}" x=${xStart} y="${yStart}" width="${width}" height="${height}"></rect>`;
            }
            function _calculateMaxDistance(xS: number, yS: number, xE: number, yE: number) {
            ...
            const mapSize: Point = self._map.getSize();
            const mapWidthFromCenter: number = mapSize.x / 2;
            const mapHeightFromCenter: number = mapSize.y / 2;
    ...
    export namespace control {
        export function reticle (opts?: L.RecticleControlOptions) {
